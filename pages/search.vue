  ...
  <v-app class="bg-cor_fundo">
    ...
    <v-main class="bg-cor_fundo">
      <v-container>
        ...
        <!-- Lista de Produtos -->
        <v-card v-if="produtos.length > 0" class="bg-cor_marron_claro rounded-xl elevation-0 mb-6">
          <v-container fluid>
            <v-row dense>
              <v-col cols="12" sm="6" md="4" lg="3" v-for="produto in produtos" :key="produto.id">
                <v-card class="rounded-xl elevation-2 card-hover" max-width="300px" outlined>
                  <v-img
                    :src="produto?.foto"
                    class="align-end"
        ...
        <!-- Lista de Lojas -->
        <v-card v-if="lojas.length > 0" class="bg-cor_marron_claro rounded-xl elevation-0">
          <v-container fluid>
            <v-row dense>
            ...
            <v-row no-gutters class="w-100 h-100">
              ...
              <v-col cols="6" class="d-flex align-center justify-center">
                <v-card class="justify-center" height="100%" width="100%">
                  ...
                  <p class="ma-5 font-weight-medium">
                    Valor: R$ {{ produtoSelecionado?.valor }}
                  </p>
                  <!-- Botão de Lixeira -->
                  <v-btn
                    v-if="produtoSelecionado?.loja.userId === currentUserId"
                    icon="mdi-trash-can"
                    class="lixo-btn"
                    color="cor_fundo"
                  ></v-btn>
...
interface Produto {
  ...
  description: string;
  valor: number;
  loja: { name: string; userId: string }; // Adicionado userId
}
...
const currentUserId = localStorage.getItem('userId'); // Ajuste conforme sua autenticação
